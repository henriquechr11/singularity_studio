...
export default function App() {
  ...
  useEffect(()=>{if(exploring||project!==null)lenisRef.current?.stop();else lenisRef.current?.start()},[exploring,project])
  useEffect(()=>{
  ...
  return <>
    <CinematicIntro lang={lang} siteRef={siteRef} onReveal={onIntroReveal}/>
    <div className={`site-shell ${introRevealed?'site-revealed':''}`} ref={siteRef} inert={!introRevealed} aria-hidden={!introRevealed}>
    ...
    <main id="main" className={paused ? 'motion-paused' : ''}>
      <section className="hero" aria-labelledby="hero-title">
        ...
        <div className="hero-visual"><BlackHole onProgress={onProgress} exploring={false} zoom={1.08} paused={paused} active={introRevealed && !exploring}/><div className="visual-coordinate">FIG. 001 — SINGULARITY<br/><span>Rₛ = 2GM / c²</span></div><button className="explore-button" onClick={openExplore} aria-label={t.explore}><span>+</span></button><div className="hole-annotation"><span className="annotation-line"/><span>EVENT HORIZON<br/><b>THE POINT OF NO RETURN.</b></span></div><span className="hero-visual-caption">[ {t.interact} ]</span></div>
